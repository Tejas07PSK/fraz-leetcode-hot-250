...
class Solution:
    ...
    def __method5 (self, k, n):
        ...
        while ((left + 1) < right):
            ...
            t1, t2 = self.__method5(k - 1, mid - 1), self.__method5(k, n - mid)
            if (t1 > t2): right = mid
            elif (t1 < t2): left = mid
            else: left = right = mid
        for i in (left, right):
            self.dp[k][n] = min(self.dp[k][n], max(self.__method5(k - 1, i - 1), self.__method5(k, n - i)))
        self.dp[k][n] += 1
        return self.dp[k][n]

    def __method5_helper (self, k, n):
        self.dp = [[None for j in range(n + 1)] for i in range(k + 1)]
        return self.__method5(k, n)

    def __method6 (self, k, n):
        dp = [[0 for j in range(n + 1)] for i in range(2)]
        for i in range(k + 1):
            l = 1
            for j in range(n + 1):
                ...
                if (j == 1):
                    dp[1][j] = 1 ; continue
                while ((l < j) and (max(dp[0][l - 1], dp[1][j - l]) > max(dp[0][l], dp[1][j - l - 1]))): l += 1
                dp[1][j] = 1 + max(dp[0][l - 1], dp[1][j - l])
            dp[0], dp[1] = dp[1], dp[0]
        return dp[0][-1]
    ...
    def superEggDrop (self, k: int, n: int) -> int:
        ...
        #return self.__method5_helper(k, n)
        #return self.__method6(k, n)
